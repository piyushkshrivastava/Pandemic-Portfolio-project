...
select continent, max(cast(total_deaths as int)) as totaldeathcount
 ...
 --breaking into datewise total cases and totaldeaths
  ...
  select cast(date as date) as date, isnull(sum(new_cases),0) as total_cases, isnull(sum(cast(new_deaths as int)),0)as total_deaths, isnull(sum(cast(new_deaths as int))/sum(new_cases)*100,0) as deathpercentage
 from PortfolioProject..CovidDeaths
 where continent is not null
 group by date
 order by 1,2
 ...
 join PortfolioProject..CovidVaccinations v
   ...
   select d.continent, d.location, d.population, isnull(v.new_vaccinations,0)
   from PortfolioProject..CovidDeaths d
 join PortfolioProject..CovidVaccinations v
 on d.location = v.location
 and d.date = v.date
 ...
 select d.continent, d.location, d.date, d.population, isnull(v.new_vaccinations,0) as new_vaccinations,
 isnull(sum(cast(v.new_vaccinations as BIGINT)) over (partition by d.location order by d.location,d.date),0) as rollingpeoplevaccinated
 from PortfolioProject..CovidDeaths d
 join PortfolioProject..CovidVaccinations v
 on d.location = v.location
 and d.date = v.date
